...
export default function DMPanel({ friendId }: { friendId: string }) {
  ...
  async function send() {
    ...
    try {
      ...
      // recarga inmediata para no esperar el polling
      load();
    } catch {}
  }
  ...
  useEffect(() => {
    ...
    timer.current = setInterval(load, 2000); // ← 2 segundos
        ...
        {msgs.map(m => (
          <div key={m.id} style={{display:"flex", justifyContent: m.mine ? "flex-end" : "flex-start"}}>
            <div className="px-3 py-2 rounded-lg" style={{background: "var(--chip)", border: "1px solid var(--border)"}}>
              <div>{m.text}</div>
              <div className="text-xs" style={{color:"var(--muted)"}}>{new Date(m.createdAt).toLocaleString()}</div>
            </div>
          </div>
        ))}
